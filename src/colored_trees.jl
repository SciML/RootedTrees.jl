...
struct ColoredRootedTree{T<:Integer,V<:AbstractVector{T},C<:AbstractVector} <: AbstractRootedTree
    ...
    function ColoredRootedTree(level_sequence::V, color_sequence::C,
                               iscanonical::Bool = false) where {T<:Integer,V<:AbstractVector{T},
                                                                 C<:AbstractVector}
...
- Terry Beyer and Sandra Mitchell Hedetniemi.
  ...
  [DOI: 10.1137/0209055](https://doi.org/10.1137/0209055)
"""
rootedtree!(level_sequence::AbstractVector, color_sequence::AbstractVector) = canonical_representation!(ColoredRootedTree(level_sequence,
                                                                                                                          color_sequence))

iscanonical(t::ColoredRootedTree) = t.iscanonical
#TODO: Validate rooted tree in constructor?

Base.copy(t::ColoredRootedTree) = ColoredRootedTree(copy(t.level_sequence), copy(t.color_sequence),
                                                    t.iscanonical)
Base.similar(t::ColoredRootedTree) = ColoredRootedTree(similar(t.level_sequence),
                                                       similar(t.color_sequence), true)
Base.isempty(t::ColoredRootedTree) = isempty(t.level_sequence)
Base.empty(t::ColoredRootedTree) = ColoredRootedTree(empty(t.level_sequence),
                                                     empty(t.color_sequence), iscanonical(t))
...
function Base.:(==)(t1::ColoredRootedTree, t2::ColoredRootedTree)
    ...
    root1_minus_root2 = first(t1.level_sequence) - first(t2.level_sequence)
    for (e1, c1, e2, c2) in
        zip(t1.level_sequence, t1.color_sequence, t2.level_sequence, t2.color_sequence)
        v1 = e1
        v2 = e2 + root1_minus_root2
        ...
        while swapped
            ...
            while subtree1_root_index <= subtree_last_index_to_sort
                ...
                if isless(subtree1, subtree2)
                    ...
                    copyto!(t.level_sequence, subtree1_root_index + order(subtree2), buffer_level,
                            1, order(subtree1))
                    ...
                    copyto!(t.color_sequence, subtree1_root_index, buffer_color,
                            order(subtree1) + 1, order(subtree2))
                    copyto!(t.color_sequence, subtree1_root_index + order(subtree2), buffer_color,
                            1, order(subtree1))
...
Base.eltype(::Type{BicoloredRootedTreeIterator{T}}) where {T} = BicoloredRootedTree{T,Vector{T},
                                                                                    Vector{Bool}}
...
function subtrees(t::ColoredRootedTree)
    ...
    return push!(subtr, ColoredRootedTree(t.level_sequence[start:end], t.color_sequence[start:end]))
end
...
function PartitionIterator(t::ColoredRootedTree{Int,Vector{Int},Vector{Bool}})
    ...
    return PartitionIterator{typeof(t),ColoredRootedTree{Int,Vector{Int},Vector{Bool}}}(t, forest,
                                                                                        skeleton,
                                                                                        edge_set,
                                                                                        edge_set_tmp)
end
