...
function Base.show(io::IO, t::RootedTree{T}) where {T}
    ...
    return show(io, t.level_sequence)
end
        ...
        while swapped
            ...
            while subtree1_root_index <= subtree_last_index_to_sort
                ...
                if isless(subtree1, subtree2)
                    ...
                    copyto!(t.level_sequence, subtree1_root_index, buffer, order(subtree1) + 1,
                            order(subtree2))
                    copyto!(t.level_sequence, subtree1_root_index + order(subtree2), buffer, 1,
                            order(subtree1))
...
function partition_forest!(forest, level_sequence, edge_set)
    ...
    return push!(forest, rootedtree(level_sequence))
end
...
function PartitionIterator(t::AbstractRootedTree)
    ...
    return PartitionIterator{typeof(t),typeof(skeleton)}(t, forest, skeleton, edge_set,
                                                         edge_set_tmp)
end
...
function PartitionIterator(t::RootedTree{Int,Vector{Int}})
    ...
    forest = PartitionForestIterator(t_forest, t_temp_forest, edge_set_tmp)
    return PartitionIterator{typeof(t),RootedTree{Int,Vector{Int}}}(t, forest, skeleton, edge_set,
                                                                    edge_set_tmp)
end
...
Base.eltype(::Type{PartitionIterator{TreeInput,TreeOutput}}) where {TreeInput,TreeOutput} = Tuple{PartitionForestIterator{TreeOutput},
                                                                                                  TreeOutput}
...
# necessary for simple and convenient use since the iterates may be modified
function Base.collect(partitions::PartitionIterator{TreeInput,TreeOutput}) where {TreeInput,
                                                                                  TreeOutput}
