...
# time wherever they appear in the Core thunk body, so they must be in scope
# even when the Core group is not selected (e.g. GROUP=QA); otherwise the file
# fails to load before any test runs. The actual call sites remain inside the
# Core thunk below.
...
run_tests(;
    ...
    # Declared `env` activates test/qa; carrying no env would also be fine here,
    # but the QA suite has its own isolated environment. Passing it via `qa=`
    # keeps it running under both GROUP=QA and the default "All" (matching the
    # original `RUN_QA_TESTS = GROUP in ("qa", "all")`).
    qa = (;
        env = joinpath(@__DIR__, "qa"), body = () -> begin
